...
public class DFSPathFinder<T> implements PathFinder<T> {
    ...
    public PathResult<T> find(Graph<T> graph, T start, T end) {
        ...
        if (!encontrado) {
            path.clear(); // Esta linea de codigo Si no se encuentra el camino, se limpia la lista de
                          // camino
        }
    ...
    private boolean dfs(Graph<T> graph, T currente, T end, List<T> visited, List<T> path) {
        ...
        for (Node<T> vecino : graph.getVecinos(currente)) {// getVecinos me va a devolver un listado de nodos b y y el
                                                           // nodo j
